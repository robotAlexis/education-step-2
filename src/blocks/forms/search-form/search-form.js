...
document.addEventListener('DOMContentLoaded', function(event){
    ...
    // Формы поиска
    document.querySelectorAll('.search-form').forEach(searchFormElement => {

        // Какая-то херня
        let Inputmask = require('inputmask');

        // Елементы ввода даты
        let dateInputElements = Array.prototype.slice.call(searchFormElement.querySelectorAll('.date-dropdown__input'));
        dateInputElements.forEach(dateInputElement => {
            dateInputElement.onfocus = openCalendar;
        });

        // Календарь
        let calendarElement = searchFormElement.querySelector('.calendar');
        // Открытие календаря
